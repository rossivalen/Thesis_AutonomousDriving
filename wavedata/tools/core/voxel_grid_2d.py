...
class VoxelGrid2D(object):
    ...
    def voxelize_2d(self, pts, voxel_size, extents=None,
        ...
        contiguous_array = np.ascontiguousarray(discrete_pts_2d).view(np.dtype((np.void, 
                                                                                discrete_pts_2d.dtype.itemsize * discrete_pts_2d.shape[1])))
        ...
        if create_leaf_layout:
            ...
            self.leaf_layout_2d[self.voxel_indices[:, 0], 0, self.voxel_indices[:, 2]] = self.VOXEL_FILLED
