...
def intersection(boxlist1, boxlist2, scope=None):
    ...
    with tf.name_scope(scope, 'Intersection'):
        ...
        all_pairs_min_ymax = tf.math.minimum(y_max1, tf.transpose(y_max2))
        all_pairs_max_ymin = tf.math.maximum(y_min1, tf.transpose(y_min2))
        intersect_heights = tf.math.maximum(
            0.0, all_pairs_min_ymax - all_pairs_max_ymin)
        all_pairs_min_xmax = tf.math.minimum(x_max1, tf.transpose(x_max2))
        all_pairs_max_xmin = tf.math.maximum(x_min1, tf.transpose(x_min2))
        intersect_widths = tf.math.maximum(
...
def iou(boxlist1, boxlist2, scope=None):
    ...
    with tf.name_scope(scope, 'IOU'):
        ...
        unions = (
            ...
            intersections)
        return tf.where(
            tf.math.equal(intersections, 0.0),
            tf.zeros_like(intersections), tf.math.truediv(intersections, unions))
